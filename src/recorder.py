...
class Recorder:
    ...
    async def _run_one(self, adapter: MarketDataAdapter) -> None:
        # Open stores and wire the raw-frame hook BEFORE connect(). The
        # subscribe handshake and any pre-snapshot frames the venue sends
        # during connect() flow through adapter.on_raw_frame and we want
        # them captured in the raw log — losing them means we can't
        # re-normalize that window if a bug is later found in the parser.
        self._open_stores(adapter)
        await adapter.connect()

        # Snapshot. The adapter has already been wired to write raw frames
        # via on_raw_frame above, so frames produced during fetch_snapshot()
        # (including the REST snapshot text) are also captured.
