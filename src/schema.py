...
class BookEvent:
    ...
    # Monotonic capture counter assigned at write time by NormalizedEventStore.
    # Used as a tiebreaker for deterministic replay ordering:
    #
    #   Coinbase: sort by (sequence ASC, event_index ASC). Sequence is
    #             primary because Coinbase guarantees contiguous sequences.
    #             event_index disambiguates events that share a sequence
    #             (e.g. a 'reset' BookEvent and the snapshot rows that follow
    #             it both carry the snapshot's sequence; the reset is emitted
    #             first so it gets the lower event_index and replays first).
    #   Bitfinex: sort by (event_time ASC, event_index ASC). Bitfinex's public
    #             book has no per-message sequence, so event_index is the
    #             only sub-millisecond tiebreaker available.
    #
    # Adapters do NOT populate this field. The store stamps it when the row
    # is persisted. None in-memory is the normal case; only rows read back
    # from parquet carry a concrete event_index.
...
BOOK_EVENT_SCHEMA = pa.schema(
    [
        ...
        # Monotonic capture counter, stamped by NormalizedEventStore at write time.
        # Used by the reconstructor for deterministic replay ordering (see
        # BookEvent.event_index for the sort policy).
