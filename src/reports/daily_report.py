...
Produces a Markdown report with data quality, model diagnostics, simulated PnL,
and an explicit accept/reject decision. Rejection reasons from the trainer are
first-class: if the model says the signal is not reliable, the report rejects it.
"""
...
def _data_quality_section(data_root: str | Path, exchange: str, symbol: str, date: datetime) -> Dict[str, Any]:
...
def _decision(
    ...
    reasons: List[str] = list(getattr(training, "rejection_reasons", []) or [])
    ...
    if backtest is not None:
        ...
        if reject_cfg.get("reject_if_stale_execution_skips", True):
            stale = int(backtest.summary.get("skipped_stale_entry", 0)) + int(backtest.summary.get("skipped_stale_exit", 0))
            if stale > 0 and backtest.summary.get("n_trades", 0) == 0:
                reasons.append(f"all candidate trades were blocked by stale/gappy execution snapshots ({stale} skips)")
...
def write_daily_report(
    ...
    decision = _decision(training, backtest, reject_cfg, dq)
    ...
    if getattr(training, "rejection_reasons", None):
        lines.append("- Trainer reliability rejection(s):")
        for reason in training.rejection_reasons:
    ...
    if training.folds:
        ...
        for f in training.folds:
            lines.append(
                f"| {f.fold_index} | {f.train_start.date()}–{f.train_end.date()} "
                f"| {f.val_start.date()}–{f.val_end.date()} | {f.n_train:,} | {f.n_val:,} "
        ...
        lines.append("_No folds — training did not produce any reliable walk-forward windows._")
        lines.append("")
    ...
    if backtest is None:
        ...
        lines.append(f"- Max snapshot delay allowed: {s.get('max_snapshot_delay_ms', 0)} ms")
        lines.append(
            f"- Trades skipped for stale snapshots: entry={s.get('skipped_stale_entry', 0)}, "
            f"exit={s.get('skipped_stale_exit', 0)}, invalid={s.get('skipped_invalid_execution', 0)}"
        )
    ...
    lines.append("---")
    lines.append("")
    lines.append("> **Reminder.** This codebase is a research environment. It does not place orders, does not connect to private APIs, and the safety guard refuses to load if `execution_enabled` is ever set to true.")
