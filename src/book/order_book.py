...
Two counters with different meanings — DO NOT confuse them:

  corruption_count : times the engine detected damage on its own (crossed
                     book, 'unknown' event, apply-time exception). A high
                     value means the venue or our parser is producing
                     inconsistent data. Worth investigating.
  resync_count     : times the book was cleanly wiped by a reset event
                     (adapter reconnect, fresh REST snapshot). This is
                     NORMAL operational behaviour. A high value here is
                     not a quality signal — venues rotate sessions all
                     the time.

The reset() method increments resync_count when it's called on a clean
book, and increments neither counter when it's called on a corrupt one
(the damage was already counted when corruption was detected).
"""
...
class OrderBook:
    ...
    def __init__(self, max_depth_tracked: int = 50) -> None:
        ...
        self._corrupt = False
        # Two separate counters with different meanings:
        #   corruption_count : times the book engine itself detected damage
        #                      (crossed book, unknown event, apply exception).
        #                      A high value here means the venue or our
        #                      parser is producing inconsistent data.
        #   resync_count     : times the book has been cleanly wiped by an
        #                      adapter-driven reset (reconnect, fresh
        #                      snapshot). This is NORMAL operational
        #                      behaviour and must not be confused with
        #                      corruption.
    ...
    def apply_event(self, ev: BookEvent) -> None:
        ...
        try:
            ...
            if ev.event_type == "snapshot":
                ...
                # Counter rules:
                #   - corruption_count: already bumped when we marked corrupt.
                #   - resync_count: NOT bumped — this snapshot is the
                #     recovery from damage we already counted, not a new
                #     clean resync. Bumping both would double-count.
                if self._corrupt:
                    self._orders.clear()
                    self._bids.clear()
                    self._asks.clear()
    ...
    def reset(self) -> None:
        ...
        Counter rules:
          - If we are currently corrupt, this reset is the recovery path
            following damage. corruption_count was already incremented when
            we entered the corrupt state; we do NOT bump anything here.
          - Otherwise this is a clean, adapter-driven resync (a reconnect,
            a fresh REST snapshot). It is normal operational behaviour and
            increments resync_count. corruption_count MUST stay where it is.

        Mixing the two counters would make the daily report read "we had 50
        corrupt periods" when in fact the venue just rotated its websocket
        50 times and nothing went wrong.
        """
        if self._corrupt:
            # Damage already counted; this reset is recovery, not a new event.
            pass
        else:
            self.resync_count += 1
        self._orders.clear()
