...
class NormalizedEventStore:
    ...
    Stamps a monotonic `event_index` on every row at write time. The index
    is the source-of-truth tiebreaker for deterministic replay ordering when
    multiple events share a timestamp (Bitfinex) or a sequence number
    (Coinbase reset + snapshot rows). See BookEvent.event_index for details.

    The counter starts at 0 for a fresh store and increments by one per
    record written, regardless of the date partition it lands in. That means
    the global write order is recoverable from the index alone, even across
    midnight UTC rotations.
    """
    ...
    def write_events(self, events: Iterable[BookEvent]) -> None:
        ...
        for ev in events:
            d = ev.to_dict()
            # Always overwrite with the store's counter. If the BookEvent came
            # in with a non-None event_index (rare; usually only in tests
            # that round-tripped through parquet) we still re-stamp so the
            # store remains the single source of truth.
            d["event_index"] = self._next_index
            self._next_index += 1
            rows.append(d)
